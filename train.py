...
if __name__ == "__main__":
    ...
    parser.add_argument("--evaluation_interval", type=int, default=200, help="interval evaluations on validation set")
...
######################################
    qwbi = 1
    f = open("/home/weidu/桌面/Yolov3_Loss.txt", 'w+')
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_i, (img_pth, imgs, targets) in enumerate(dataloader):
            ...
            # log_str += AsciiTable(metric_table).table    #format是格式化函数
            log_str += "\nTotal loss {}".format(loss.item())

            #打印损失，输出到txt文件，画曲线图
            print(qwbi)
            print("第",qwbi,"次的损失:",loss.item())
            print >>f,"第",qwbi,"次的损失:",loss.item()
            Loss_List.append(loss.item())
            count_List.append(qwbi)
            qwbi = qwbi + 1
    ...
    print(count_List)
    data_dict = {}
    for i, j in zip(count_List, Loss_List):
        data_dict[i] = j
    ...
    plt.legend()
    plt.savefig(r"/home/weidu/桌面/yolo3loss.png")
    plt.show()
